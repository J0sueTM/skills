...
- **⚡ Verify facts vs source. No guess.** Column name, enum value, API field → check real source of truth, no assume plausible name. Guessed identifier = latent bug. (This session: aiosql returning op = `<!`, not `<!>` I assumed — read adapter source, no just try.)
- **⚡ No depend on serialization-format order for meaning.** JSON objects unordered; `jsonb` reorders keys (length, then bytes). Loaded-from-storage = no rely on declaration order. Order encode meaning (dependency chain between steps) → derive from data relationships (resolve each step from declared input), not key-appearance order. Round-trip through storage = moment hidden order-dependency surface as bug.
...
- **Plain public names** for module surface. Private prefix only for actual internal. Push back hard on wall of `_`-prefixed helpers — most want inline, not hide.
...
- **Lookup helper = `by_<field>`, not `for_`.** `fetch_alarms_by_event`, not `..._for_event` — name read as query it runs.
...
- **⚡ Name file for what it produces, not mechanism.** Service create speed-alarm decisions = `speed_alarm_decision` — not `speed_policy` (thing it uses), not `policy_run` (how). Stuff no feed that result → own box: decision + alarm DB writes → own `decision`/`alarm` services, named for what *they* produce. Name by end result = cohesion honest + anticipate axis of variation: next alarm kind get own file, no swell this one. (Separate real *stages* — find/run/save + orchestrator on top — but no shred below that; unit of extraction = responsibility, not few lines.)
...
- **⚡ Build by transformation, not accumulation.** Comprehension / recursion that *returns* value > loop mutate accumulator (`append`, then `del` from pending set). Order of dependent steps = recursion on each step declared input, not mutate-until-stable loop over worklist. "simpler, but no mutation" → pure version usually also shorter, once stop reach for loop.
...
- **Make value serializable; no hand-roll serializer.** Result persist/send → push capability into type (pydantic model self-serialize `model_dump_json`), no `_serialize_result` walking it field by field. Hand-rolled walker drift from shape; model can't. Same instinct as reuse canonical util: no reimplement what framework / type system already give.
...
- **⚡ Docstring/comment earn place only saying what code can't — test subtractive.** Companion to rule above: line restate signature+body (`save_alarm` → "grava alarme e devolve id") → delete, even public surface. Keep only invisible *why*: why fallback exist (`ON CONFLICT DO NOTHING` returns no id on redelivery), why failure isolated (one bad policy no sink others), what window anchored to. Restated docstring = noise age into lie when code move on without it.
